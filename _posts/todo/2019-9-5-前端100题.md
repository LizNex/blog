...
parseInt的第二个参数是基数，map会把index作为第二参数传入parseInt
...
<html lang="en">
  ...
  <body>
    ...
    <script>
      ...
      document.querySelector('#input').addEventListener('keydown',throttle(sayHi,2000))
    </script>
  </body>
</html>
```
...
回调：回调地狱，逻辑读起来复杂，中间不能return
promise：链式调用容易理解，但是中途不能暂停promise,错误用回调函数捕获
Generator: 可控制函数执行，配合co库使用
Async/Await:代码清晰，不用写then链，处理回调地狱
